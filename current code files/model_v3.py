...
from metablockv2 import MetaBlock

class SkinEffnetB5(nn.Module):
    def __init__(self, pretrained=True, feature_extract=True, use_metablock=False, dropout_p = 0.0, num_classes=8, meta_num=15):
        super().__init__()
        ...
        # Capture the number of features output by the backbone (2048 for B5)
        ...
        # MetaBlock grouping
        self.comb_feat_maps = 32 # number of groups to split 2048 channels into before MetaBlock, must divide 2048; 32 metadata-controlled feature groups
    ...
    def forward(self, img, meta_data = None):
        ...
        if self.use_metablock: # if using MetaBlock
            ...
            x = x.view(B, C, H, W) # restore shape, x shape = [B, 2048, H, W]
            
        x = nn.functional.adaptive_avg_pool2d(x, 1) # x shape = [B, 2048, 1, 1]
        x = torch.flatten(x, 1) # flatten for the classifer layer, x shape = [B, 2048]
        x = self.dropout(x)
